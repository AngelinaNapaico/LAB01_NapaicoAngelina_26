...
cd ms-productos
docker build -t valencia123/ms-productos:latest .

# Construir imagen ms-pedidos
cd ms-pedidos
docker build -t valencia123/ms-pedidos:latest .
```
...
```bash
docker login
docker push valencia123/ms-productos:latest
docker push valencia123/ms-pedidos:latest
```
